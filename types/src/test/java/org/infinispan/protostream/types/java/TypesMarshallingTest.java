...
public class TypesMarshallingTest {
    ...
    public void testFloatArray() throws IOException {
        ImmutableSerializationContext ctx = newContext();
        float[] array = floatArray();
        byte[] data = ProtobufUtil.toWrappedByteArray(ctx, array, 512);
        float[] copy = ProtobufUtil.fromWrappedByteArray(ctx, data);
        assertArrayEquals(array, copy, 1);
    }

    @Test
    public void testBoxedFloatArray() throws IOException {
        doArrayMarshallTest(boxedFloatArray(), useWrappedMessage());
    }
    ...
    private static Float[] boxedFloatArray() {
